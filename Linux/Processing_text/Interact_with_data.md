...
*Cú pháp* : 
```
grep [option] partten [file]
```
VD: 
- Tim 1 file chứa các từ : 
```
grep "word" filename
```
...
```
- Tìm kiếm số lần xuất hiện của từ 
```
...
- Thay thế 1 chuỗi: 
```
sed 's/<string-replace>/<string-des>/' geekfile.txt
``` 
VD: 
```
sed 's/unix/linux/' geekfile.txt
```
- Thay thế dựa vào số lân xuất hiện của dòng: 
```
sed 's/<string-replace>/<string-des>/<number> geekfile.txt
``` 
VD: 
```
...
```
- 3g thay thế theo lần xuất hiện của từ trong dòng 
VD: 
```
echo "Welcome To The Geek Stuff" | sed 's/\(\b[A-Z]\)/\(\1\)/g'
```
- Thay thế chuỗi ở 1 dòng cụ thể 
VD: 
```
...
*Ví dụ*:
- In ra từng dòng của file: 
```
awk '{print;}' file.txt
```
- In ra những dòng có chuỗi mẫu: 
```
awk '/Thomas/' file.txt
```
- In ra những trường nhât định: 
```
...
Cú pháp :
```
awk 'BEGIN {print "Name\tDesignation\tDepartment\tSalary";} END {print "Report Generated\n-----------------";}' file.txt'
```
- Phép so sánh: 
```
awk '$1 > 200' file.txt
